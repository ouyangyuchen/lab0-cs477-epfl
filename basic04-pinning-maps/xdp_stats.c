...
static int stats_poll(const char *pin_dir, int map_fd, const struct bpf_map_info *old_info, int interval)
{
	int map_type = old_info->type;
	int old_id = old_info->id;
	...
	while (1) {
		...
		if (old_id != info.id) {
			// the pinnned map has been reloaded, print aborts
			close(stats_map_fd);
			return 0;
		}
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		err = stats_poll(pin_dir, stats_map_fd, &info, interval);
		close(stats_map_fd);
		if (err < 0)
			return err;
	}
