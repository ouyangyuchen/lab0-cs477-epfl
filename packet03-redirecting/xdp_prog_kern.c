...
static __always_inline void swap_src_dst_mac(struct ethhdr *eth)
{
	...
	memcpy(buf, eth->h_dest, sizeof(buf));
	memcpy(eth->h_dest, eth->h_source, sizeof(buf));
	memcpy(eth->h_source, buf, sizeof(buf));
}
...
static __always_inline void swap_src_dst_ipv6(struct ipv6hdr *ipv6)
{
	...
	struct in6_addr buf;
	memcpy(&buf, &ipv6->daddr, sizeof(buf));
	memcpy(&ipv6->daddr, &ipv6->saddr, sizeof(buf));
	memcpy(&ipv6->saddr, &buf, sizeof(buf));
}
...
static __always_inline void swap_src_dst_ipv4(struct iphdr *iphdr)
{
	...
	__be32 buf;
	memcpy(&buf, &iphdr->daddr, sizeof(buf));
	memcpy(&iphdr->daddr, &iphdr->saddr, sizeof(buf));
	memcpy(&iphdr->saddr, &buf, sizeof(buf));
}
...
int xdp_redirect_func(struct xdp_md *ctx)
{
	...
	int eth_type;
	int action = XDP_PASS;
	unsigned char dst[ETH_ALEN] = {0x5e,0x83,0xac,0xbc,0x0b,0x41};	/* Assignment 2: fill in with the MAC address of the left inner interface */
	...
	memcpy(eth->h_dest, dst, sizeof(dst));
...
int xdp_router_func(struct xdp_md *ctx)
{
	...
	if (h_proto == bpf_htons(ETH_P_IP)) {
		...
		fib_params.flowinfo	= *(__be32 *) ip6h & (0x0FFFFFFF);
